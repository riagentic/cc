...
import { pct, tildePath, tokens, usd } from "../lib/format.ts";
import { Dot, Elapsed, Meter, Pill, Stat } from "./parts.tsx";
import {
  IconAgents,
  IconAlert,
  IconBranch,
...
export function StatusStrip(): VNode {
  ...
  const measured = session.turns > 0;
  ...
  const elsewhere = session.cwd !== null && session.cwd !== project?.path &&
  ...
  return (
    <div class="strip">
      <Stat label="Project" clamp title={project?.path}>
        ...
        {elsewhere && (
          <span
            ...
            title={`The running session is in ${session.cwd} — a project switch takes effect on the next start`}
          >
            session elsewhere
          </span>
        )}
      ...
      <Stat label="Model">
        {IconModel({ size: 14 })}
        <span class="truncate">{session.model ?? workspace.model}</span>
      </Stat>
      ...
      <Stat
        label={session.turnStartedAt !== null ? "Running" : "Last turn"}
        numeric
      ...
      >
        ...
        <Elapsed
          ...
          fallbackMs={session.lastTurnMs}
        />
      </Stat>

      <Stat label="Cost" numeric title="Session total reported by the CLI">
        {IconCoins({ size: 14 })}
        <span>{usd(session.cost)}</span>
      </Stat>

      <Stat label="Session">
        <Dot status={session.status} />
        <span class="truncate">
          {holds > 0 ? "waiting for you" : session.status}
        </span>
      </Stat>
